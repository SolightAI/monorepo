...
def compare_html_files(file1_content: str, file2_content: str) -> str:
    ...
    file1_lines = [
        line for line in cleaned_file1_content.splitlines() if line.strip()
    ]  # remove empty lines
    file2_lines = [
        line for line in cleaned_file2_content.splitlines() if line.strip()
    ]  # remove empty lines
...
def _parse_result_from_html_diff(result: str) -> bool:
    match = re.search(
        r"<answer>(USER_AUTHENTICATED|USER_LOGGED_OUT|UNKNOWN)</answer>", result
    )
...
async def check_is_logged_in_using_html_diff(
    ...
    result: str = (
        await AGENT_CLIENT.ainvoke(
            [HumanMessage(content=PROMPT.format(html_diff=html_diff))]
        )
    ).content  # type: ignore
...
async def check_is_logged_in(
    ...
    with NamedTemporaryFile(
        ...
        logger.info(
            f"[{task_id}] Login check result: {'Logged in' if is_logged_in else 'Not logged in'}"
        )

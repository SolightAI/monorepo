...
def fill_placeholder(
    history: list[AgentHistory], placeholder: str, content: str
) -> None:
    for step_number, step_history in enumerate(history):
        ...
        step_history.model_output.current_state.next_goal = (
            step_history.model_output.current_state.next_goal.replace(
                placeholder, content
            )
        )
        # -------------------------------------------------------------------------
        ...
        for i, action_result in enumerate(step_history.result):
            if not step_history.model_output.action or i >= len(
                step_history.model_output.action
            ):
                continue
            ...
            action_taken = next(
                (
                    action_name
                    for action_name in action_model.model_fields_set
            ...
            setattr(
                step_history.model_output.action[i],
                action_taken,
                type(action_params_obj)(**action_parameters),
            )
            # ----------------------------------------------------------
            ...
            if action_result.extracted_content:
                action_result.extracted_content = (
                    action_result.extracted_content.replace(placeholder, content)
                )
...
def decorator_factory(
    history: list[AgentHistory], placeholder: str
...
def _enable_cached_generation_for_history_rerun(
    agent: Agent, history: list[AgentHistory]
) -> list[AgentHistory]:
    """
    ...
    GENERATION_TASKS = {
        ...
        for _tool in TOOLS
        if _tool.__name__.startswith("generate")
    }

    placeholders: dict[
        str, dict[str, str | int]
    ] = {}  # {output: {"fn": generation_fn, "id": "0"}}
    ...
    for step_number, step_history in enumerate(history):
        ...
        for key, value in placeholders.items():
            step_history.model_output.current_state.next_goal = (
                step_history.model_output.current_state.next_goal.replace(
                    key,
                    PLACEHOLDER_FORMAT.format(tool_name=value["fn"], index=value["id"]),
                )
            )
        # -------------------------------------------------------------------------
        ...
        for i, action_result in enumerate(step_history.result):
            ...
            action_taken = next(
                ...
                None,
            )  # we always limit to one action per step
            ...
            for key, value in placeholders.items():
                for k, v in action_parameters.items():
                    # FIXME: this works only for strings, not for other types
                    if isinstance(v, str):
                        action_parameters[k] = v.replace(
                            key,
                            PLACEHOLDER_FORMAT.format(
                                tool_name=value["fn"], index=value["id"]
                            ),
                        )
            setattr(
                step_history.model_output.action[i],
            ...
            if action_taken in GENERATION_TASKS:
                ...
                placeholders[_new_placeholder] = {
                    ...
                    "id": len(
                        placeholders
                    ),  # some unique id in case a generation task is called multiple times
                }

                # registering new decorated function
                new_action_fn = decorator_factory(
                    history,
                    PLACEHOLDER_FORMAT.format(
                        tool_name=action_taken, index=len(placeholders) - 1
                    ),
                )(GENERATION_TASKS[action_taken])
                new_action_fn.__name__ = (
                    new_action_fn.__name__ + "_" + str(len(placeholders))
                )
                agent.controller.action(GENERATION_TASKS[action_taken].__doc__)(
                    new_action_fn
                )

                setattr(
                    step_history.model_output.action[i], action_taken, None
                )  # deactivate the original function call
                new_action_to_call = {
                    new_action_fn.__name__: type(action_params_obj)(**action_parameters)
                }  # create the new function call
                actions_to_execute = (
                    step_history.model_output.action[i].model_dump(exclude_unset=True)
                    | new_action_to_call
                )
                ...
                step_history.model_output.action[i] = NewActionModel(
                    **actions_to_execute
                )
            # -------------------------------------------------------------------------
            ...
            if action_result.extracted_content:
                for key, value in placeholders.items():
                    action_result.extracted_content = (
                        action_result.extracted_content.replace(
                            key,
                            PLACEHOLDER_FORMAT.format(
                                tool_name=value["fn"], index=value["id"]
                            ),
...
async def _rerun_history(
    ...
    logger.info(
        f"[{task_id}] Replaying history with {len(history.history)} steps ({max_retries=}, {skip_failures=}, {fallback_to_llm=}, {max_fallbacks=}, {delay_between_actions=})"
    )

    history.history = _enable_cached_generation_for_history_rerun(
    ...
    for i, history_item in enumerate(history.history):
        ...
        logger.info(
            f"[{task_id}] Replaying step {i + 1}/{len(history.history)}: goal: {goal}"
        )
        ...
        skip_action = False
        while (
            skip_action is False
            and retry_count < max_retries
            and number_of_failures < max_fallbacks
        ):
            try:
                # Capture browser state before executing the actions for this history_item
                browser_state_before_action = await agent.browser_context.get_state(
                ...
                if i < len(history.history) - 1:
                    ...
                    if (
                        ...
                        and model_output.current_state.evaluation_previous_goal.startswith(
                            "Failed - "
                        )
                    ):
                        logger.info(
                            f"[{task_id}] Next action evaluated current action as failed, skipping current action"
                        )
                        skip_action = True
                        continue

                step_action_results = await agent._execute_history_step(
                    history_item, delay_between_actions
                )

                if any(
                    [_action.error for _action in step_action_results]
                ):  # if any action failed, raise an error (triggering the retry on agent logic)
                    raise RuntimeError(f"Step {i + 1} failed: {step_action_results}")

                results.extend(
                    step_action_results
                )  # Accumulate overall results for the function's return value
                ...
                if history_item.model_output:
                    ...
                    if step_action_results:
                        for r_idx, r_item in enumerate(step_action_results):
                            ...
                            if action_models_for_this_step and r_idx < len(
                                action_models_for_this_step
                            ):
                                ...
                                action_name = next(
                                    (
                                        name
                                        for name in action_model.model_fields_set
                                        if name != "index"
                                    ),
                                    None,
                                )
                            ...
                            if not is_done_action_result and r_item.include_in_memory:
                                if r_item.extracted_content:
                                    msg_content = "Action result: " + str(
                                        r_item.extracted_content
                                    )
                ...
                if history_item.model_output and browser_state_before_action:
                    replayed_browser_state_history = BrowserStateHistory(
                        ...
                        interacted_element=AgentHistory.get_interacted_element(
                            ...
                            browser_state_before_action.selector_map,
                        ),
                ...
                if retry_count < max_retries:
                    logger.warning(
                        f"[{task_id}] Step {i + 1} failed (attempt {retry_count}/{max_retries}), retrying..."
                    )
                ...
                if max_fallbacks > 0 and number_of_failures >= max_fallbacks:
                    raise RuntimeError(
                        f"Reached the maximum number of failures: ({number_of_failures}/{max_fallbacks})"
                    )
                ...
                if fallback_to_llm:
                    ...
                    await agent.step(
                        AgentStepInfo(
                            ...
                            max_steps=len(history.history) + 10,
                        )
                    )  # '+10' to prevent llm from using the 'done' action

                    if agent.state.last_result and agent.state.last_result[0].error:
                        raise RuntimeError(
                            f"Step {i + 1} failed after {max_retries} attempts and falling back on LLM: {str(e)}"
                        )

                    if (
                        agent.state.last_result
                        and agent.state.last_result[0].success is False
                    ):
                        raise RuntimeError(
                            f"Step {i + 1} failed after {max_retries} attempts and falling back on LLM: {str(e)=} {agent.state.last_result=}"
                        )
                    ...
                    state_message = AgentMessagePrompt(
                        state,
                        result,  # type: ignore (is this really unbound?)
                    ...
                    model_output = await agent.get_next_action(
    ...
    await agent.step(
        AgentStepInfo(step_number=len(history.history), max_steps=len(history.history))
    )
...
@observe()
async def try_rerun_from_history(
    config: Config,
    task_id: str,
    identifier: str,
    ...
    **kwargs: Any,
    ...
    with NamedTemporaryFile(mode="w+", suffix=".json", delete=False) as history_file:
        config.s3_client.download_file(f"{identifier}/history.json", history_file.name)
        ...
        try:
            history = AgentHistoryList.load_from_file(
                history_file.name, agent.AgentOutput
            )

            history = await _rerun_history(
                agent,
                history,
                max_retries=3,  # cost nothing to retry, cost a lot to fail
                skip_failures=False,
                delay_between_actions=2,  # leaves time for the page to load (otherwise leads to errors)
                max_fallbacks=max(
                    5, int(len(history.history) * 0.33)
                ),  # 1/3 of the actions or 5, whichever is greater
            )

            return history, True
        except Exception as e:
            logger.info(
                f"[{task_id}] Couldn't run cached history, running agent again, {e=}"
            )

...
def replace_click_element_by_click_element_by_index() -> None:
    ...
    print(
        f"Successfully replaced click_element by click_element_by_index in {file_to_change}"
    )
...
def prevent_agent_to_use_content_from_extract_page_to_click_element() -> None:
    ...
    to_replace_second_element = (
        f"msg = f'📄  Extracted from page\\n: {{content}}\\n' + {warning_message}"
    )
    ...
    if not modified_first_element:
        raise RuntimeError(
            f"Couldn't modify first element of the file in {file_to_change}"
        )

    if not modified_second_element:
        raise RuntimeError(
            f"Couldn't modify second element of the file in {file_to_change}"
        )

    shutil.move(temp_file, file_to_change)

    print(
        f"Successfully prevented agent to use content from extract_page to click_element in {file_to_change}"
    )

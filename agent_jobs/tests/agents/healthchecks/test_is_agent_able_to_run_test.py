...
from src.common.dto import Test, TestCategory
from src.agents.healthchecks import is_agent_able_to_run_test
...
ERROR_MESSAGE = (
    "Expected {expected_result} but received {result}. Explanation: {explanation}"
)
...
class TestTickPick:
    ...
    async def test_signup_wo_email_verification(self, task_id: str) -> None:
        ...
        assert is_able is True, ERROR_MESSAGE.format(
    ...
    async def test_signup_w_email_verification(self, task_id: str) -> None:
        ...
        assert is_able is False, ERROR_MESSAGE.format(
    ...
    async def test_login_with_google_no_secrets(
        self, task_id: str, repeat: int
    ) -> None:
        test = Test(
            category=TestCategory.SMOKE,
...
class TestSolight:
    ...
    async def test_login_with_invalid_social_media(
        self, task_id: str, social_media: str
    ) -> None:
        test = Test(
            category=TestCategory.SMOKE,

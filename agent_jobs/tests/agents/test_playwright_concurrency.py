...
@pytest.mark.asyncio
async def test_use_playwright_in_parallel(
    task_id: str, playground_url: str, valid_username_password_credentials: dict
) -> None:
    """Test authentication with valid username/password on the simple login page using subprocesses."""

    url = f"{playground_url}/auth/email_password/simple"
    ...
    try:
        ...
        processes = []
        logger.info(
            f"Starting {num_sessions} parallel Playwright sessions for URL: {url}"
        )
        ...
        for i, process in processes:
            ...
            assert (
                "Failed to connect to socket" not in stdout.strip()
            ), f"Session {i} failed: {stdout.strip()}"
            assert (
                "Failed to connect to socket" not in stderr.strip()
            ), f"Session {i} failed: {stderr.strip()}"
    ...
    assert (
        execution_time > 3
    ), f"Test took only {execution_time} seconds to run, something is wrong"

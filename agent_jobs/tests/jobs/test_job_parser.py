...
async def test_parse_validate_url_job() -> None:
    ...
    payload = {
        ...
        "job_id": "1234567890",
        "payload": {"url": "https://tickpick_dev:tickpick.1@dev.tickpick.com/"},
    }
...
async def test_parse_generate_tests_job() -> None:
    ...
    payload = {
        ...
        "payload": {
            ...
            "feature": {
                ...
                "name": "Tick Pick",
                "description": "Tick Pick is a web-based ticketing system that allows users to book tickets for events, meetings, and other activities.",
                "access_conditions": {"must_be_logged_in": True},
                "dependents": [
                    {
                        "id": "1234567890",
                        "urls": [
    ...
    assert job.job_id == "1234567890"
    assert (
        job.payload.product.url == "https://tickpick_dev:tickpick.1@dev.tickpick.com/"
    )
    assert job.payload.product.name == "Tick Pick"
    assert (
        job.payload.product.description
        == "Tick Pick is a web-based ticketing system that allows users to book tickets for events, meetings, and other activities."
    )
    assert job.payload.product.documentation == "https://tickpick.com/docs"
    assert job.payload.categories == ["SMOKE", "UNIT"]
...
async def test_unknown_job_type() -> None:
    ...
    payload = {"job_type": "unknown_job_type", "job_id": "1234567890", "payload": {}}

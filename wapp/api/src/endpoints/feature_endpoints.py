...
async def get_feature_endpoint(feature_id: UUID4, current_user=Depends(get_current_user)) -> FeatureSchema:
    return await get_feature(feature_id)


@router.post("/")
async def create_feature_endpoint(feature: FeatureCreateSchema, current_user=Depends(get_current_user)) -> FeatureSchema:
    return await create_feature(feature)


@router.delete("/{feature_id}")
async def delete_feature_endpoint(feature_id: UUID4, current_user=Depends(get_current_user)) -> dict:
...
async def update_feature_endpoint(feature_id: UUID4, feature_update: FeatureUpdateSchema, current_user=Depends(get_current_user)) -> FeatureSchema:
